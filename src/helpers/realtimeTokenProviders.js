...
// #1480 / #1624), and the callers that rely on it — meeting prepare/start and
// dictation streaming — resolve their provider ids in meetingTranscriptionRouting.js
// and dictationStreamingRouting.js respectively. Dependencies are injected so the
// table is unit-testable without Electron.
//
// Phenisys fork adaptation: the OpenWhispr Cloud layer (account/login/Cloud) has
// been removed, so the `openwhispr` cloud-token branches are intentionally absent.
// BYOK mode is the only supported mode; non-BYOK requests fail with a clear error
...
const REALTIME_TOKEN_PROVIDERS = {
  "assemblyai-realtime": async ({ environmentManager, proxyFetch }, options, streams) => {
  ...
  "deepgram-realtime": async ({ environmentManager }, options, streams) => {
  ...
  "openai-realtime": async ({ environmentManager }, options, streams) => {
    // Phenisys fork: BYOK-only. The OpenWhispr Cloud client-secret flow was removed
    // with the account/Cloud layer; a missing key is the only failure mode.
