 ...
 */
export default function LocalNotesTree({ onNewNote, onShowStructureIntro }: LocalNotesTreeProps) {
  ...
  const privateSpace = useMemo(() => spaces.find((s) => s.kind === "private") ?? null, [spaces]);
    ...
    <div className="flex-1 overflow-y-auto px-2 pb-4">
      <div className="flex items-center gap-1 pr-1">
        ...
        >
          <ChevronRight
            size={12}
            className={cn("shrink-0 text-foreground/30 transition-transform duration-150", "rotate-90")}
          />
      ...
      {onShowStructureIntro && (
        ...
        >
          {t("notes.tree.structureIntroHint", { defaultValue: "Afficher l'aide sur les notes" })}
        </button>
      )}
    </div>
  );
