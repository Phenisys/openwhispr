...
export default function PersonalNotesView({
	...
	const isSidePanelLayout =
		isMeetingMode || (isNarrowWindow && activeNoteId != null);
		...
		(writes: PendingNoteWrite[], reason: PendingSaveReason) => {
			for (const write of writes) {
				void window.electronAPI
					.updateNote(write.noteId, write.updates)
					.catch((err: unknown) => {
						logger.warn(
	...
	const transitionToNote = useCallback(
		(
			...
			reason: Extract<PendingSaveReason, "switch" | "overview">,
		) => {
			const pending = takePendingSnapshots();
	...
	const effectiveModelId = useSettingsStore(
		(s) => selectResolvedNoteFormatting(s).model,
	);
	const { isComplete: isOnboardingComplete, complete: completeOnboarding } =
		useNotesOnboarding();
	...
	const diarizationSessionId = useMeetingRecordingStore(
		(s) => s.diarizationSessionId,
	);
	const recordingNoteId = useMeetingRecordingStore((s) => s.recordingNoteId);
	const sessionDiarizationEnabled = useMeetingRecordingStore(
		(s) => s.sessionDiarizationEnabled,
	);
	const sessionExpectedCount = useMeetingRecordingStore(
		(s) => s.sessionExpectedCount,
	);
	const userTouchedStepper = useMeetingRecordingStore(
		(s) => s.userTouchedStepper,
	);
	...
	const overviewSpace = useMemo(
		() =>
			activeContext
				? (spaces.find((s) => s.id === activeContext.spaceId) ?? null)
				: null,
	...
	const editorFolders = useMemo(
		() =>
			activeNote
				? folders.filter((f) => f.space_id === activeNote.space_id)
				: folders,
		[activeNote, folders],
	);

	const [calendarEventName, setCalendarEventName] = useState<string | null>(
		null,
	);
	useEffect(() => {
		...
		window.electronAPI
			.gcalGetEvent?.(activeNote.calendar_event_id)
			.then((result) => {
				setCalendarEventName(
					result?.success && result.event?.summary
	...
	const handleDelete = useCallback(
		async (id: number) => {
			if (
				shouldCancelPendingSavesForDelete(draftRef.current?.noteId ?? null, id)
			) {
	...
	const isEnhancementStale = useMemo(() => {
		if (!editorEnhancedContent || !activeNote?.enhanced_at_content_hash)
			return false;
	...
	useEffect(() => {
		if (
			!meetingRecordingRequest ||
			activeNoteId !== meetingRecordingRequest.noteId
		)
			return;
		const note =
			activeNote?.id === meetingRecordingRequest.noteId ? activeNote : null;
		const seedSegments = note?.transcript
			? parseTranscriptSegments(note.transcript)
			: [];
	...
	const isActiveNoteRecording =
		isTranscribing && recordingNoteId === activeNote?.id;
			...
			<div className="flex-1 flex flex-col min-w-0 min-h-0">
				{editorNote ? (
					<>
							...
							actionPicker={
								<ActionPicker
									onRunAction={async (action) => {
										...
										const isLiveNote = liveNoteId === activeNote?.id;
										const rawTranscript =
											(isLiveNote ? liveTranscript : "") || activeNoteRawTranscript;
										...
										if (rawTranscript) {
											...
											const segments =
												isLiveNote && liveSegments.length > 0
													? liveSegments
													: parseTranscriptSegments(rawTranscript);
											if (segments.length > 0) {
												isMeetingNote = true;
												// Renamed speakers can live only in the mapping table, not on
												// every segment, so it must be consulted here too.
												const mappings =
													await window.electronAPI?.getSpeakerMappings?.(
														editorNote.id,
													);
						...
						/>
						<ActionManagerDialog
							open={showActionManager}
							onOpenChange={setShowActionManager}
						/>
					</>
