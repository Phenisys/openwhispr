// MAX_SPEAKER_COUNT mirrors ../constants/speakerDetection.json; imported
// inline (not via JSON import) so node:test's ESM loader can run this module.
const MAX_SPEAKER_COUNT = 8;
...
export async function saveUploadNote({
  ...
  if (!res.success || !res.note) return res;

  if (noteUpdates) {
    await window.electronAPI.updateNote(res.note.id, noteUpdates as Record<string, string>);
  }
  return res;
}
