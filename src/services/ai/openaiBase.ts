...
export function resolveSelfHostedOpenAIBase(configuredBaseUrl: string): string {
  ...
  if (!normalized || !isSecureEndpoint(normalized)) {
...
// Legacy alias used by ReasoningService streaming + openai provider; resolves
// the configured Custom endpoint for the current cleanup provider.
