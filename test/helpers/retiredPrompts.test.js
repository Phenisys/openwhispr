...
test("current-hash snapshot matches the live locale bundles (ratchet)", async () => {
  ...
  // chatAgent fallback lives in registry.ts as a JS string concatenation; read
  // it from source because Node's native type-stripping can't resolve the
  // extensionless import chain (same convention as promptsRegistry.test.js).
  const registrySrc = fs.readFileSync(
    path.join(__dirname, "..", "..", "src", "config", "prompts", "registry.ts"),
    "utf8"
  );
  const m = registrySrc.match(/DEFAULT_CHAT_AGENT_PROMPT\s*=\s*([\s\S]*?);\n\n/);
  assert.ok(m, "DEFAULT_CHAT_AGENT_PROMPT extractable from registry.ts");
  const expr = m[1].replace(/;\s*$/, "");
  expected.chatAgent = sha256(new Function(`return (${expr});`)());
